...
public class ProductVirtualGrid : MonoBehaviour
{
    ...
    private void Rebuild()
    {
        ...
        totalRows = Mathf.CeilToInt(
            (float)products.Count / columns
        );
    ...
    private void UpdateContentSize()
    {
        ...
        float height =
            ...
            Mathf.Max(0, totalRows - 1) * verticalSpacing +
            bottomPadding;
    ...
    private void UpdateVisibleCards(bool force)
    {
        ...
        firstVisibleRow = Mathf.Max(
            0,
            firstVisibleRow - extraRows
        );

        if (!force &&
            firstVisibleRow == previousFirstRow)
        {
            return;
        }
        ...
        int visibleRowCount = Mathf.CeilToInt(
        ...
        int lastIndex)
    {
        ...
        foreach (KeyValuePair<int, ProductCard> pair
                 in activeCards)
        {
            if (pair.Key < firstIndex ||
                pair.Key > lastIndex)
            {
    ...
    {
        if (!activeCards.TryGetValue(
                index,
                out ProductCard card))
        {
            return;
        }
    ...
    {
        // Connect your existing image loader here.
    }
}